...
abstract class REST_Controller extends CI_Controller
{
	...
	{
		// If CORS Support enabled, set the correct header output.
    if ($this->config->item('rest_x_domain')) {

      if ($this->config->item('allow_cendentials')) {
        header('Access-Control-Allow-Credentials: true');
        
        if (isset($_SERVER['HTTP_REFERER'])) {
          $url_components = parse_url($_SERVER['HTTP_REFERER']);
          if (in_array($url_components['host'], $this->config->item('allowed_origins'))) {
